...
class LeafModel(object):
  ...
  '''
  def fit(self, A, Y, weights, fit_init=None, num_features=2, **kwargs):
    n_obs = len(Y)
    n_items = int(A.shape[1]/num_features)
    self.n_items = n_items
    Y_bool = np.zeros((n_obs, n_items))
    Y_bool[range(n_obs),Y] = 1
    self.Ypred = np.mean(Y_bool, axis=0)
    return(0)
  ...
  def predict(self, A, *args,**kwargs):
    return np.tile(self.Ypred, (A.shape[0], 1))
...
#   errors = (1-Ypred[(np.arange(Y.shape[0]),Y)])**2.0
  ...
  def to_string(self,*leafargs,**leafkwargs):
    return "Y reg pred: " + str(self.Ypred)
